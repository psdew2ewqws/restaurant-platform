...
export class AlertingService {
  ...
  async sendAlert(alert: Alert): Promise<void> {
    try {
      ...
      console.log(`Alert sent: ${alert.title} to channels: ${targetChannels.join(', ')}`);
  ...
  private async sendEmailAlert(channel: AlertChannel, alert: Alert): Promise<void> {
    ...
    console.log(`📧 EMAIL ALERT: ${alert.title} to ${channel.config.recipients?.join(', ')}`);
  ...
  private async sendSlackAlert(channel: AlertChannel, alert: Alert): Promise<void> {
    console.log(`💬 SLACK ALERT: ${alert.title} to ${channel.config.channel}`);
  ...
  private async sendSmsAlert(channel: AlertChannel, alert: Alert): Promise<void> {
    console.log(`📱 SMS ALERT: ${alert.title} to ${channel.config.recipients?.join(', ')}`);
  ...
  private async sendTeamsAlert(channel: AlertChannel, alert: Alert): Promise<void> {
    console.log(`👥 TEAMS ALERT: ${alert.title} to ${channel.config.channel}`);
...
Environment: ${alert.environment}`;
  ...
  private async sendFallbackAlert(error: Error, originalAlert: Alert): Promise<void> {
    try {
      console.error(`🚨 ALERTING SYSTEM FAILURE - Original alert: ${originalAlert.title}`);
      ...
      console.error('Even fallback alerting failed:', fallbackError);
    }
  }
