-- ==================================================
-- RESTAURANT PLATFORM LICENSE SYSTEM
-- PostgreSQL-Centric Implementation inspired by Picolinate
-- ==================================================
...
-- ==================================================
-- INDEXES FOR PERFORMANCE (Following Picolinate patterns)
-- ==================================================
...
BEGIN
    ...
    SELECT 
        (value->>'license_id')::uuid,
        (value->>'company_id')::uuid,
        'expired',
        value,
        now()
...
BEGIN
    ...
    new_notifications AS (
        ...
        SELECT 
            ...
            CASE 
                WHEN days_remaining <= 7 THEN 'URGENT: Your ' || type || ' license expires in ' || days_remaining || ' days!'
                WHEN days_remaining <= 14 THEN 'WARNING: Your ' || type || ' license expires in ' || days_remaining || ' days.'
                ELSE 'NOTICE: Your ' || type || ' license expires in ' || days_remaining || ' days.'
            END,
            CASE 
...
CREATE OR REPLACE FUNCTION get_company_license_status(company_uuid uuid)
RETURNS jsonb AS $$
DECLARE
    result jsonb;
BEGIN
...
-- ==================================================
-- SCHEDULED TASKS (PostgreSQL Cron Jobs)
-- ==================================================

-- Schedule license expiry checks every 6 hours
SELECT cron.schedule(
    'license-expiry-check',
    '0 */6 * * *', -- Every 6 hours
    'SELECT check_expired_licenses();'
);

-- Schedule days remaining updates every hour
SELECT cron.schedule(
    'license-days-update',
    '0 * * * *', -- Every hour
    'SELECT update_license_days_remaining();'
);

-- Schedule expiry warnings daily at 9 AM
SELECT cron.schedule(
    'license-expiry-warnings',
    '0 9 * * *', -- Daily at 9 AM
    'SELECT create_license_expiry_warnings(30);'
);
...
('Basic Plan', 'Small restaurant package', 'basic', 365, 10, 2, '["basic_pos", "advanced_reports", "customer_management", "inventory_basic"]', '{"monthly": 49, "yearly": 499}'),
...
('Enterprise Plan', 'Large restaurant enterprise', 'enterprise', 365, 100, 20, '["all_features"]', '{"monthly": 299, "yearly": 2999}')
